...
function top_open_whitelist_ui() {
    ...
    if (domain === "" && document.location.href.indexOf("feed:https") === 0) {
   ...
   //the user we can't run on it.
    ...
    btns[adblock_default_button_text] = {
      ...
      click: function() {
        safari.extension.dispatchMessage("add_custom_filter", {"url": generateUrl()});
        safari.self.addEventListener("message", function() {
          if (event.name === "add_custom_filter_response" && $("#reload_page").is(":checked")){
